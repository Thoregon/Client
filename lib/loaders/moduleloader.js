...
class ModuleLoader extends Loader {
    ...
    async doStart() {
        // await this.loadSysDependencies();   // will be done when first client connects
    ...
    async isModuleOutdated(module, cache) {
        ...
        const distmodified = await getModified(DIST + module);
        if (!isValidDate(distmodified)) return true;
        const modified = distmodified > cachmodified;
        if (modified) debuglogML2(">> ModuleLoader - module needs update", module);
        return modified;
    }

    cacheModule(module, cache, modified) {
        const lastModified = formatDate(isValidDate(modified) ? modified : new Date());
